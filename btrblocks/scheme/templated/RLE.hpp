...
extern template size_t compress_len(INTEGER* out_lengths,
  ...
  public:
    ...
    static inline u32 compressColumn(const NumberType* src,
      ...
      size_t res;
      BTR_IFELSEARM_SVE({
        res = compress_sve(rle_count.data(), rle_values.data(), src, nullmap, stats.tuple_count);
      }, {
        res = compress_len(rle_count.data(), rle_values.data(), src, nullmap, stats.tuple_count);
      });
